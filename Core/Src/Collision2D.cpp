...
bool IsCollision(const Circle2D* circle, const Rect2D* rect)
{
	Vec2f minPos = rect->GetMin();
	Vec2f maxPos = rect->GetMax();
	Vec2f closest(GameMath::Clamp<float>(circle->center.x, minPos.x, maxPos.x), GameMath::Clamp<float>(circle->center.y, minPos.y, maxPos.y));
...
bool IsCollision(const OrientedRect2D* orientedRect0, const OrientedRect2D* orientedRect1)
{
	...
	Mat2x2 roateMat0(GameMath::Cos(rotate0), -GameMath::Sin(rotate0), GameMath::Sin(rotate0), GameMath::Cos(rotate0));

	float rotate1 = orientedRect1->rotate;
	Mat2x2 roateMat1(GameMath::Cos(rotate1), -GameMath::Sin(rotate1), GameMath::Sin(rotate1), GameMath::Cos(rotate1));
...
void OrientedRect2D::GetInterval(const Vec2f& axis, float& outMin, float& outMax) const
{
	...
	Mat2x2 roateMat(GameMath::Cos(rotate), -GameMath::Sin(rotate), GameMath::Sin(rotate), GameMath::Cos(rotate));
	for (int32_t index = 0; index < vertices.size(); ++index)
	{
