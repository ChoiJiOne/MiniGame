...
{
	std::wstring currSystemTime = CrashManager::Get().GetCurrentSystemTimeAsString();
	std::wstring crashDumpPath = CrashManager::Get().GetCrashDumpPath();
	...
	std::wstring path = GameUtils::PrintF(L"%sWindows-%s-Minidump.dmp", crashDumpPath.c_str(), currSystemTime.c_str());
...
std::wstring CrashManager::GetCurrentSystemTimeAsString()
{
...
std::wstring CrashManager::GetCrashDumpPath()
{
